Shader "ProjectionSprayAI/Unlit/ShowUv2"
// Shader "シェーダー名"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct v2f
            {
                ...
                float3 worldPos : TEXCOORD1; // 特にTEXCOORDは高精度で保持する際に利用する(COLORの場合[0,1]の低精度データに利用)
            ...
            {			
                ...
                v.uv2.y = 1.0f - v.uv2.y;
            #endif
                float4 pos0 = UnityObjectToClipPos(v.vertex);    //Unityの組み込み関数(CGIncludes/UnityShaderUtilities.cgincを参照)
                                                                //ModelViewProjection行列をかけることでスクリーン同次座標へ変換する
                                                                //mul(UNITY_MATRIX_MVP, float4(pos, 1.0))と同等なイメージ
                float4 pos1 = float4(v.uv2*2.0f-1.0f,0.0f,1.0f);//lightUVの位置を取得
                                                                //適切な設定ができていれば, 各メッシュそれぞれについて, 重ならないようにlightUVが設定されているはず
                v2f o;
                o.vertex   = lerp(pos0,pos1,_T);//通常の位置とlightUVの位置を線形補間, スライダーを動かすと滑らかにlightUVへ移動するはず
                o.uv2      = v.uv2;
                o.worldPos = mul(unity_ObjectToWorld,v.vertex).xyz; //Unityの組み込み変数(CGIncludes/HLSLSupport.cgincを参照), World変換行列(4x4)
            ...
            {
                // sample the texture
                return half4(i.uv2,0.0f,1.0f);//UVカラーを描画
            }
