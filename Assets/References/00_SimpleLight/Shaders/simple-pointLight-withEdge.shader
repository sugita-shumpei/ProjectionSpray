Shader "ProjectionSprayAI/Unlit/Simple/PointLight-Reciever-Edge"
// Shader "シェーダー名"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag (v2f i) : SV_Target//出力を表すSEMANTICS, 一つの場合 SV_Target, 複数の場合, 構造体の各要素にSV_Target0, SV_Target1とつける
            {
                ...
                half4 col = max(0.0f,atten) * _LitCol;
                return col;
            }
        ...
        Pass
        {
            ...
            {
                v2f o;
                o.vertex   = UnityObjectToClipPos(_EdgeSize*v.vertex);//Unityの組み込み関数(CGIncludes/UnityShaderUtilities.cgincを参照)
                                                                      //ModelViewProjection行列をかけることでスクリーン同次座標へ変換する
                                                                      //mul(UNITY_MATRIX_MVP, float4(pos, 1.0))と同等なイメージ
                return o;
            }
